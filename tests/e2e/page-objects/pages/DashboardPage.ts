...
export class DashboardPage extends BasePage {
  ...
  async filterByStatus(status: 'All Requests' | 'New' | 'Viewed' | 'Quoted' | 'Accepted' | 'Scheduled' | 'Completed'): Promise<void> {
    logger.log(`Filtering dashboard by status: "${status}"...`);
  ...
  async toggleEmergencyFilter(): Promise<void> {
    logger.log('Toggling the "Emergencies Only" filter...');
  ...
  async switchToTableView(): Promise<void> {
    logger.log('Switching to Table view...');
  ...
  async switchToMapView(): Promise<void> {
    logger.log('Switching to Map view...');
  ...
  async goToAdminDashboard(): Promise<void> {
    logger.log('Navigating to admin dashboard...');
  ...
  async verifyOnCustomerDashboard(): Promise<void> {
    logger.log('Verifying on customer dashboard...');
  ...
  async getVisibleRequests(): Promise<Array<{title: string, status: string}>> {
    logger.log('Getting visible quote requests...');
  ...
  async findRequestByTitle(title: string): Promise<boolean> {
    logger.log(`Looking for request with title: ${title}`);
  ...
  async openRequestDetailsByIndex(index = 0): Promise<void> {
    logger.log(`Opening request details for request ${index}...`);
  ...
  async verifyRequestExists(title: string, expectedStatus = 'new'): Promise<void> {
    logger.log(`Verifying request exists: ${title} with status: ${expectedStatus}`);
  ...
  async waitForRequestsToLoad(timeout = 10000): Promise<void> {
    logger.log('Waiting for requests to load...');
  ...
  async refreshDashboard(): Promise<void> {
    logger.log('Refreshing dashboard...');
  ...
  async openRequestDetails(requestId: string): Promise<void> {
    logger.log(`Opening request details for request ID: ${requestId}...`);
  ...
  async verifyRequestDetails(expectedData: {
    ...
    logger.log('Verifying request details in modal...');
    ...
    logger.log('✅ Request details verified successfully');
  }
  ...
  async closeRequestDetailsModal(): Promise<void> {
    logger.log('Closing request details modal...');
  ...
  async verifyRequestCreationDate(): Promise<void> {
    logger.log('Verifying request creation date...');
    const modal = this.requestDetailModal;
    ...
    logger.log('✅ Request creation date verified');
  }
  ...
  async navigateToRequestInTable(requestId: string, userType: 'user' | 'admin'): Promise<void> {
    logger.log(`Navigating dashboard table to find request ${requestId}...`);
    ...
    logger.log(`✅ Found request ${requestId} in dashboard table.`);
  }
  ...
  async findAndOpenRequest(requestId: string, userType: 'user' | 'admin'): Promise<void> {
    logger.log(`Finding request ${requestId} on the ${userType} dashboard...`);
    const locator = userType === 'admin'
        ? `div[data-request-id="${requestId}"]`
        : `button[data-request-id="${requestId}"]`;
    ...
    logger.log('✅ Found and opened request modal.');
  }
  ...
  async openRequestById(requestId: string, userType: 'user' | 'admin'): Promise<void> {
    logger.log(`Opening request ${requestId} from ${userType} dashboard...`);
    ...
    logger.log(`✅ Successfully opened request ${requestId}.`);
  }
}
