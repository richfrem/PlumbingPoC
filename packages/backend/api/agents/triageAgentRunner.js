// packages/backend/api/agents/triageAgentRunner.js
// Runner for TriageAgent workflow using YAML definition for intelligent request analysis
...
try {
  ...
  // Fallback for bundled/Netlify environment
  __dirname = process.cwd();
  console.log('[TriageAgentRunner] Using process.cwd() as __dirname:', __dirname);
}

// Try multiple paths to find the YAML file
let YAML_PATH = path.resolve(__dirname, '../../../../agents/triage-agent.yaml');
...
try {
  ...
  console.log('[TriageAgentRunner] Successfully loaded YAML config from:', YAML_PATH);
} catch (error) {
  console.error('[TriageAgentRunner] Failed to load YAML config. Tried paths:', {
    path1: path.resolve(__dirname, '../../../../agents/triage-agent.yaml'),
    path2: path.resolve(process.cwd(), 'agents/triage-agent.yaml'),
    path3: path.resolve(__dirname, '../../../../../agents/triage-agent.yaml'),
    path4: path.resolve(process.cwd(), '../../../../agents/triage-agent.yaml'),
    cwd: process.cwd(),
    __dirname
  });
 ...
 */
export async function runTriageAnalysis(requestData) {
  if (!openAiClient) {
    throw new Error('OpenAI API key not configured');
  }

  console.log('[TriageAgentRunner] Starting triage analysis for request:', requestData.id);
...
Return your analysis as a JSON object.`;
  ...
  try {
    ...
    console.log('[TriageAgentRunner] Analysis completed:', {
    ...
    console.error('[TriageAgentRunner] Error during analysis:', error);
    throw error;
  }
}
